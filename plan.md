...
| G19 | LOW | `timescale/hypertable.py` | `get_stats` divides by `after_compression_total_bytes` (0 → DB error); `number_partitions` power-of-2 unvalidated. | Guard division; validate partitions |
| G20 | LOW | `expressions/graph_filters.py` | Compound `[vector, threshold]` detection is an `isinstance(value[0], list)` heuristic; thresholds silently default (1.0/0.0). | Tighten value validation + explicit errors |
